...
class Opauth{
	...
	public function __construct($config = array(), $run = true){
		...
		if ($run && (!isset($this->config['stop']) || $this->config['stop'] == false)) $this->run();
	}
	...
	public function callback(){
		...
		switch($this->env['callback_transport']){
			case 'session':
				$CI =& get_instance();
				$CI->load->library('session');//session_start();
				$response = $CI->session->userdata('opauth');//$response = $_SESSION['opauth'];
				$CI->session->unset_userdata('opauth');//unset($_SESSION['opauth']);
				break;
			case 'post':
